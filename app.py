...
# FAISS
# Parse FAISS_ENABLED from environment
FAISS_ENABLED_ENV = os.environ.get("FAISS_ENABLED", "true").lower() in ("1", "true", "yes")

# Initial value - will be adjusted in optimize_for_memory_profile()
FAISS_ENABLED = FAISS_ENABLED_ENV
...
def create_openai_client():
    ...
    # Adjust timeout based on RAM profile
    timeout_seconds = 30.0 if IS_LOW_RAM else 60.0
...
# Configure LRU cache based on RAM profile
def configure_embedding_cache():
    """Configure embedding cache based on RAM profile"""
    flags = UpgradeFlags.get_all_flags()
    cache_size = flags["EMBEDDING_CACHE_SIZE"]
    
    # Clear existing cache and reconfigure
    embed_text.cache_clear()
    
    # Create new wrapper with appropriate cache size
    import functools
    cached_embed_text = functools.lru_cache(maxsize=cache_size)(embed_text.__wrapped__)
    
    # Update the module's reference
    globals()['embed_text'] = cached_embed_text
...
# Thêm vào sau dòng 315 (sau MAX_EMBEDDING_CACHE_SIZE = ...)
...
def configure_embedding_cache():
    ...
    # Get the original function (bypassing the decorator)
    try:
        original_func = embed_text.__wrapped__
    except AttributeError:
        # If __wrapped__ doesn't exist, use the function directly
        import functools
        original_func = embed_text
    ...
    # Create new wrapper with appropriate cache size
    import functools
    cached_embed_text = functools.lru_cache(maxsize=cache_size)(original_func)
...
@lru_cache(maxsize=None)  # Will be reconfigured in optimize_for_memory_profile()
